using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class PlayerShuriken : MonoBehaviour {
	Rigidbody2D rig;
	public float throwForce;
	public float damage;
	public float headShotMultiplier;
	public int MAX_SHURIKEN_BOUNCE= 6;
	public int NINJA_POWERFUL_BONUS_BOUNCE=10;
    public GameObject sparkPrefab;
	int currentBounceCount;
	public Transform shurikenExplosion;
	// Use this for initialization
	void Start () {
		currentBounceCount=MAX_SHURIKEN_BOUNCE;
	
	

	}
	public void fly(Vector2 direction){
	//	Debug.Log("direction  "+direction);
		if (GameController.currentAbility==PlayerController.PlayerAbility.POWERFUL){
			MAX_SHURIKEN_BOUNCE+=NINJA_POWERFUL_BONUS_BOUNCE;
		}
		GetComponent<Rigidbody2D>().velocity =  throwForce *direction.normalized;


	}
	// Update is called once per frame
	void Update () {
		GetComponent<Rigidbody2D>().velocity = GetComponent<Rigidbody2D>().velocity.normalized * throwForce;
	
	
	}
	public void destroyShuriken(){
        if (GameController.currentAbility==PlayerController.PlayerAbility.BOMB){
            Instantiate(shurikenExplosion, transform.position, Quaternion.identity);
            GameObject.FindWithTag("GameController").GetComponent<GameController>().shakeCam();
        }
		
		Destroy(gameObject);

	}
	void OnCollisionEnter2D(Collision2D coll) {
		if (coll.gameObject.tag=="wall" || coll.gameObject.tag=="ground" ||coll.gameObject.tag=="bounceItem"){
			if (currentBounceCount>0){

				currentBounceCount--;
                Instantiate(sparkPrefab, new Vector3(coll.contacts[0].point.x,coll.contacts[0].point.y,0), Quaternion.identity);

				if (currentBounceCount==0) destroyShuriken();
				AudioManager.instance.playShurikenBounceSound();
			}
		}
		else 	if (coll.gameObject.tag==""){
			destroyShuriken();

		} else if (coll.gameObject.tag == "bombBox"){
				coll.gameObject.GetComponent<BombBox>().blow();
				destroyShuriken();
		}else	if (coll.gameObject.tag=="woodenBox" ){
			coll.gameObject.GetComponent<WoodenBox> ().breakBox();
			destroyShuriken ();
		
		}else if (coll.gameObject.tag == "hostage"){
			coll.gameObject.GetComponent<Hostage>().winAnimation();
			destroyShuriken();
		}else if (coll.gameObject.tag == "canon"){
			coll.transform.parent.gameObject.GetComponent<Canon>().shoot();
			destroyShuriken();
		}

	}

	void OnTriggerExit2D(Collider2D other)
	{

	
	}
	void OnTriggerEnter2D(Collider2D other) { 
		
		Debug.Log("shuriken trigger  "+other.name);
	
		if (other.gameObject.transform.parent.gameObject.tag == "iceCube"){
			other.gameObject.transform.parent.gameObject.GetComponent<IceCube> ().breakTheIce ();

		//	AudioManager.instance.playSound(AudioManager.instance.soundIceBreak);
		//	Destroy(other.gameObject.transform.parent.gameObject);
		}
	}

}
